    ...
    const mergeByProvince = (a1, a2, a3, annee) => {
        ...
        a1.map(itm => {
          ...
          let newObject = {
            ...
            "Incendie": a3.find((item) => (item.Juridiction === itm.Juridiction) && item)[annee],
            "Dommage": a2.find((item) => (item.Juridiction === itm.Juridiction) && item)[annee],
            "Superficie": itm.km
      ...
      annees.forEach(annee => {
        ...
        donnesCombinee.push({"annee": annee, "data": mergeByProvince(superficieProvincesAnnee,valDommagesAnnee, nbreIncendieAnnee, annee)})
      });
    ...
    figure.selectAll('circle')
        ...
        .join(enter => enter.append('circle')
            ...
            update => update.transition(transition().duration(500).ease(easeLinear))
                        .attr('cx', d => nbrIncendiesScale(d.Incendie))
                        .attr('cy', d => valDommagesScale(d.Dommage)).attr('r', d=> superficieProvincesScale(d.Superficie)),
            exit => exit.remove())
}
